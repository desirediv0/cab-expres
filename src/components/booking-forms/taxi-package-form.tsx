...
import { taxiPackageSchema, type TaxiPackageForm } from "@/lib/validations";
import { FormField, inputClass, submitButtonClass } from "./form-field";

export function TaxiPackageFormComponent() {
  const [loading, setLoading] = useState(false);
  const { register, handleSubmit, reset, formState: { errors } } = useForm<TaxiPackageForm>({
    resolver: zodResolver(taxiPackageSchema),
  });

  const onSubmit = async (data: TaxiPackageForm) => {
    setLoading(true);
    try {
      const fd = new FormData();
      Object.entries(data).forEach(([k, v]) => fd.append(k, v));
      fd.append("formType", "Taxi Package");
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      ...
      <FormField label="To City" error={errors.toCity?.message}>
        <input {...register("toCity")} className={inputClass} placeholder="Enter to city" />
      </FormField>
